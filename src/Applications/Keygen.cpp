...
int main(int argc, char **argv)
{
  ...
  QSharedPointer<AsymmetricKey> privkey(lib->CreatePrivateKey());

  QSharedPointer<CppDsaPrivateKey> base_priv_key = privkey.dynamicCast<CppDsaPrivateKey>();
  Integer generator = base_priv_key->GetGenerator();
  Integer modulus = base_priv_key->GetModulus();
  Integer subgroup = base_priv_key->GetSubgroup();

//FIXME: Hard coded for generating DSA key pairs. 
//Below loop is not generic. 
  int count = 0;
  while(count < key_count) {

    QSharedPointer<AsymmetricKey> privkey = QSharedPointer<AsymmetricKey>(new CppDsaPrivateKey(modulus, subgroup, generator));
    QSharedPointer<AsymmetricKey> pubkey(privkey->GetPublicKey());
    QByteArray hvalue = hash->ComputeHash(pubkey->GetByteArray());
    QString id = Integer(hvalue).ToString();

    if(!privkey->Save(privdir_path + QDir::separator() + id)) {
